...
let subst =
  ...
  let man =
    ...
    ; `P {|This command is only meant to be called when a user pins a package to
           its development version. Especially it replaces $(b,1.0.1) strings
    ...
    ; var "VERSION_NUM" "same as $(b,1.0.1) but with a potential leading \
                         'v' or 'V' dropped"
...
let default =
  ...
  , Term.info "dune" ~doc ~version:"1.0.1"
      ~man:
        [ `S "DESCRIPTION"
